...
<?php
	...
	$retrieve_query = mysqli_query($connections,"SELECT * FROM requesttbl where stat='2' ORDER BY rollNo DESC ");
	...
	$db_PL = $row_requesters["priority_level"];
		...
		<tr>
			...
			<td width='10%'><center><a href='?jScript=$jScript && newScript=$new_Script && view_Request=$get_Update && ver_0x=$roll&&$get_Delete&&redir_=$notif1 ' class='view'>View</a></center></td>
			...
			<td width='20%'>
				<center>
					...
					<a href='	?jScript=$jScript && newScript=$new_Script && get_Accomplish=$get_Accomplish && ver_03=$roll&&$get_Update&&redir_=$notif1 ' class='accom'>Accomplished</a>
...
<?php
	...
	if(empty($_GET['get_Accomplish'])){
		...
		$acom = mysqli_query($connections, "SELECT * FROM requesttbl where rollNo='$roll' ");
	
		$rowAcom = mysqli_fetch_assoc($acom);
		
		$db_Requester = $rowAcom["requester"];
		
		
		
				if(isset($_POST["accomplish"])){
		
			mysqli_query($connections, "UPDATE requesttbl SET stat=3 WHERE rollNo = '$roll' ");
	...
	<div id="modal">
		<div class="modal-content">
			<div class="close" onclick="closeB()" style="cursor:pointer;">X</div>
				...
				<font style="text-shadow:0 2px 1px #fff, 0 -2px 1px #fff, 2px 0 1px #fff, -2px 0 1px #fff;" color="black"><b><?php echo "Is "?></br><font style="text-shadow:0 2px 1px #fff, 0 -2px 1px #fff, 2px 0 1px #fff, -2px 0 1px #fff;" size="5em" color="blue"><?php echo $db_Requester . "'s";?><br></font><?php echo "request accomplished?"; ?></b></font>
