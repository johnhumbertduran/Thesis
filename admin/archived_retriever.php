...
<?php
	...
	$retrieve_query = mysqli_query($connections,"SELECT * FROM requesttbl WHERE stat='0' ORDER BY rollNo DESC ");
		...
		<tr>
			...
			<td>
				<center>
					...
					<a href='	?jScript=$jScript && newScript=$new_Script && get_Update=$get_Update && ver_01=$roll&&$get_Delete ' class='updateBtn'>Unarchive</a>
	...
	if(empty($_GET['get_Update'])){
		...
		$db_Requester = $rowNum["requester"];
				...
				if(isset($_POST["update"])){
								...
								mysqli_query($connections, "UPDATE requesttbl SET stat = '1' WHERE rollNo = '$roll' ");
		...
		<div class="modal_content">
			<div class="close" onclick="closeB()" style="cursor:pointer;">X</div>
				<form method="POST">
							...
							<h1 class="label">Unarchived</h1>

                            <br>

				            <font style="text-shadow:0 2px 1px #fff, 0 -2px 1px #fff, 2px 0 1px #fff, -2px 0 1px #fff;" color="black"><b><?php echo "Do you want to unarchive "?></br><font style="text-shadow:0 2px 1px #fff, 0 -2px 1px #fff, 2px 0 1px #fff, -2px 0 1px #fff;" size="5em" color="blue"><?php echo $db_Requester;?></font><?php echo " ?"; ?></b></font>
					...
					<input type='submit' value='Unarchive' name='update' class='updateBtn' style='border:none; padding:8px; font-size:1em; cursor:pointer;'> <a  class='deleteBtn' style=" padding:8.5px; font-size:1em; cursor:pointer;" onclick="closeB()">Cancel</a>
				</form>
...
<?php
	...
	if(empty($_GET['get_Delete'])){
		...
		$db_Requester = $rowNum["requester"];
			...
			mysqli_query($connections, "DELETE FROM requesttbl WHERE rollNo = '$roll' ");
